...
class Stionic_Endpoint_Users extends WP_REST_Controller {
	...
	public function register_api_hooks() {
		...
		// Sign out
		register_rest_route( $this->namespace, '/m_users/logout', array(
			array(
				'methods'         => 'POST',
	...
	function logout(){
		...
		if ($authHeader) {
			...
			if($token) {
				...
				$accepts = get_user_meta( $user_id, '_stionic_jwt_accept', true );
				update_user_meta( $user_id, '_stionic_jwt_accept', array_filter($accepts, function($i){ return $i != $token; }) );
				return true;
			}
		}
		return false;
